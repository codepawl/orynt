...
- Grounded create chunks are materialized as deterministic TypeScript/Vitest test scaffolds when they do not include explicit content.
- Scaffolds include an explicit `Generated by Openpawl` marker.
- Validation remains explicit and writes are strictly constrained to new allowed test files.

### v0.1.0-alpha.9

- External installability cut.
- Repo-root `openpawl.config.json` discovery.
- Reusable workflow and copyable sample workflow.
- Install docs and safer write-mode defaults.

### v0.1.0-alpha.10

- Exact `@openpawl` mention-trigger UX for maintainer comments.
- Mention commands support `review`, `plan`, `add tests`, and `fix failing tests`.
- Mention-triggered runs stay dry-run only.
- Workflow invocation now calls `@codepawl/cli` directly:
  - `bun --filter @codepawl/cli dev -- openpawl-trigger ...`
  - `bun --filter @codepawl/cli dev -- run ...`
- Live verification passed:
  - issue `@openpawl review`: run `27208458149`
  - PR opened baseline: run `27208687623`
  - PR `@openpawl add tests`: runs `27208690487` and `27208692054`
- Pre-fix failure `27206619272` was caused by workflow runtime args being routed through Turbo via `bun run dev:cli`; alpha.10 keeps the migration-safe direct package invocation path.
...
| Gamma | `v0.1.0-alpha.4` | Safe write-mode v1 with scaffolded create-only test-file writes and deterministic templates |
...
**Historical note:** This section records the 2026-06-08 implementation state. Its `bun run dev:cli` workflow examples are historical; the current alpha.10 workflow invokes `@codepawl/cli` directly with `bun --filter @codepawl/cli dev -- openpawl-trigger ...` and `bun --filter @codepawl/cli dev -- run ...`.
...
- At this point, the workflow called the monorepo CLI script: `bun run dev:cli -- run ...`.
...
- [`.github/workflows/openpawl.yml`](.github/workflows/openpawl.yml) — Reusable GitHub Actions workflow:
  ...
  - Historical alpha.1 invocation: `bun run dev:cli -- run`
...
- Grounded create chunks without model-provided content materialize deterministic TypeScript/Vitest scaffolds.
...
- Scaffolds include an explicit `Generated by Openpawl` marker and preserve applied-file artifacts on failures.
...
2. No diff-style autonomous patch generation yet; write-mode remains scoped to safe create-only test scaffolding.
...
### 2026-06-09 Openpawl alpha.10 mention-trigger verification

**Scope**: Verified the alpha.10 `@openpawl` mention-trigger UX live after switching workflow runtime invocations away from the root Turbo alias and into direct `@codepawl/cli` package calls.

**Current workflow invocation path**:
- Trigger resolution uses `bun --filter @codepawl/cli dev -- openpawl-trigger ...`.
- Agent execution uses `bun --filter @codepawl/cli dev -- run ...`.

**Pre-fix failure**:
- `27206619272` failed because workflow runtime args were routed through Turbo via `bun run dev:cli`.
...
**Limitations**:
- Mention-triggered runs remain dry-run only.
- Write mode remains manual-only via `workflow_dispatch`.
