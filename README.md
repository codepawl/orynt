...
Openpawl currently targets **v0.1.0-alpha.9**.
...
- [0.1.0-alpha.4](CHANGELOG.md): write-mode v1 scaffold generation for grounded create chunks, including safe test-file creation and deterministic TypeScript/Vitest scaffolds.
- [0.1.0-alpha.9](CHANGELOG.md): first external installability cut with repo-root config, workflow template, install docs, and safer write-mode defaults.
...
- v0.1.0-alpha.9: first external installability cut (repo-root config, install docs, workflow template, safer defaults).
...
Openpawl loads `openpawl.config.json` from the repo root when present. The write-mode validation command lives there for external installs. See [docs/OPENPAWL_INSTALL.md](docs/OPENPAWL_INSTALL.md) for the copy/paste setup.

Created test files are scaffolded from minimal deterministic templates when no file content is provided by the model. For TypeScript files, the scaffold is a valid Vitest skeleton that includes the `Generated by Openpawl` marker.
...
In write mode, create chunks are converted into filesystem scaffolds before validation. Safe grounded TypeScript/Vitest create chunks receive deterministic skeleton content when no replacement content is available.
...
- No diff-style autonomous patch generation yet; write mode stays constrained to safe create-only scaffolds under allowed test paths.
