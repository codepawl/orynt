# CodePawl Desktop Private Beta Release Smoke
...
2. Confirm `dist/private-beta/codepawl-desktop-0.1.0-linux-x64.tar.gz` and `dist/private-beta/SHA256SUMS` exist.
3. Extract the archive in a clean temporary directory.
4. Confirm `codepawl-desktop`, `codepawl-runner/scripts/desktop-repository-run.mjs`, `codepawl-runner/scripts/register-extensionless-esm-loader.mjs`, `RELEASE_MANIFEST.json`, and this smoke checklist are present.

## Runtime Smoke

- App launch: run `./codepawl-desktop` from the extracted directory and confirm the desktop shell opens.
...
- Provider readiness: add the Codex CLI provider reference and run preflight. Expected ready state requires `codex` on `PATH`; missing CLI should fail closed with a visible reason.
- Repository run: select a local git repository, enter a repository task, and start a run only after provider readiness passes.
...
- Local app data: `$XDG_CONFIG_HOME/com.codepawl.desktop` or `~/.config/com.codepawl.desktop`.
...
Quit the app, archive the app data directory if debugging evidence needs to be preserved, then delete `$XDG_CONFIG_HOME/com.codepawl.desktop` or `~/.config/com.codepawl.desktop`. Relaunching should show first-run onboarding again and an empty local run list.
