...
| Settings | partial | Desktop settings show plan quota, usage ledger metric, permission mode, connectors, and account shell; Tauri `settings_get`/`settings_update` now persist local settings, provider key references, and a private-beta retention policy under the app data directory. | Provider refs are durable references only; no OS keychain-backed raw secret storage or release-channel settings are implemented. | Add keychain-backed provider setup, release channel settings, and diagnostics/data-location controls. |
...
| Tests | partial | Unit/package, desktop, Tauri, real repository-run sidecar, fake walkthrough, local persistence, restart reload, unsafe path rejection, corrupted manifest handling, and local Chromium marketing smoke pass. | No packaged desktop smoke, real Tauri backend E2E, installer test, hardened artifact-open test, or real-provider preflight test. | Add release gate covering packaged app start, local repo run, provider preflight, hardened artifact review, and secret scan. |
...
| `pnpm --filter @codepawl/desktop test` | pass | 1 file, 39 tests passed, including selected repository path submission, persisted run listing after restart, and reopened event/artifact evidence. |
...
| `pnpm test:tauri` | pass | Rust/Tauri unit wrapper passed 17 tests, including repository path validation, durable persistence write/read, restart reload, settings/provider refs, unsafe path rejection, and corrupted manifest handling. |
