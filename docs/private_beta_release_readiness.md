...
The strongest implemented path is now the Tauri-owned repository run slice: the desktop UI accepts a selected local git repository path and task prompt, Tauri validates local path boundaries, invokes the local Coding Apprentice runner through a sidecar boundary, emits real lifecycle events to the UI, writes a local artifact manifest with contract, event log, verifier input/result, redacted log, memory store, and replay plan references, persists repository-run snapshots under the app data directory, and blocks real runs until a provider reference passes preflight. Package-level walkthroughs still cover the broader fake/controlled Codex path. The product remains incomplete because packaged distribution, hardened artifact browsing, updater/signing, full beta onboarding, and real OS keychain-backed provider storage are still missing or partial.
...
| --- | --- | --- | --- | --- |
| Desktop shell | partial | `apps/desktop/src/App.tsx` renders a chat-first workspace, selected repository path input, provider setup controls, beta-unavailable surface chips, and streamed run lifecycle notices; `pnpm --filter @codepawl/desktop test` passed with 41 tests. | Settings/memory/skill panels still lean on mock state, and artifact opening/review is not implemented. | Desktop can review/open persisted artifacts from the real run and reduce remaining mock-only surfaces. |
...
| Tests | partial | Unit/package, desktop, Tauri, real repository-run sidecar, fake walkthrough, provider-reference/preflight/run-blocking coverage, local persistence, restart reload, unsafe path rejection, corrupted manifest handling, and local Chromium marketing smoke pass. | No packaged desktop smoke, real Tauri backend E2E, installer test, hardened artifact-open test, or real provider/API invocation smoke. | Add release gate covering packaged app start, local repo run, provider preflight, hardened artifact review, and secret scan. |
...
2. Durable local persistence now exists for repository-run snapshots and settings, but automatic cleanup, hardened artifact opening, and mock-to-real memory/skill management remain incomplete.
...
1. Harden persisted evidence review.
   - Add Tauri-owned artifact open/read commands with path validation, missing-file errors, and redacted display for contract, verifier input/result, redacted logs, memory candidates, and replay plans.
...
| `pnpm --filter @codepawl/desktop test` | pass | 1 file, 41 tests passed, including selected repository path submission, provider setup/preflight UI, missing-provider run blocking, persisted run listing after restart, and reopened event/artifact evidence. |
...
| `pnpm test:tauri` | pass | Rust/Tauri unit wrapper passed 21 tests, including repository path validation, durable persistence write/read, restart reload, provider reference save/load/delete, provider preflight ready/failed states, missing-provider run blocking, raw-secret non-persistence, unsafe path rejection, and corrupted manifest handling. |
