# Orynt Private Beta Release-Readiness Audit

Audit date: 2026-07-04

## Verdict

Orynt now has a first real private-beta desktop slice for a supervised, repository-scoped Coding Apprentice. It is still not a complete private beta desktop product for real users.
...
| Provider setup | partial | Desktop settings can save, list, test, and delete a local Codex CLI provider reference through Tauri IPC. Tauri preflight detects `codex` on `PATH` and checks the existing session with `codex login status`; Orynt does not start browser OAuth, device-code login, or access-token login from the app. Orynt records ready/failed status plus preflight reasons and blocks repository runs unless a selected model/provider passes preflight. First-run onboarding states the local Codex CLI/provider-readiness requirement. Tests cover save/load/delete, failed preflight, successful mocked preflight, detect-only auth guidance, run blocking, and raw-secret non-persistence. | It depends on Codex CLI's credential cache and does not implement an Orynt-owned OS keychain item for raw secrets. Model/provider diagnostics are still basic. | Keep the CLI-auth-only boundary explicit in beta onboarding; add richer diagnostics around Codex login status and credential-store configuration. |
| Onboarding | working | Desktop now shows first-run private-beta onboarding for new local app state, persists dismissal under `orynt:private-beta-onboarding:v1` with legacy `codepawl:private-beta-onboarding:v1` read compatibility, explains repository-only scope, local-first app data/artifact storage, Codex CLI provider readiness, approval/evidence workflow, and disabled browser/desktop/files/terminal/cloud/billing surfaces. Composer start-run guards block until onboarding is dismissed, a repository path is selected, and provider readiness passes. | The copy is renderer-side and not yet paired with a packaged-app first-run smoke. | Preserve this conservative beta boundary during packaging and add a packaged first-run smoke. |
...
3. Provider setup is partial: local Codex CLI session detection, status persistence, and preflight gating exist, but Orynt-owned OS keychain-backed raw-secret storage and richer provider diagnostics are not implemented.
...
| `pnpm test:tauri` | pass | Rust/Tauri unit wrapper passed 31 tests, including repository path validation, durable persistence write/read, restart reload, provider reference save/load/delete, provider preflight ready/failed states, missing-provider run blocking, raw-secret non-persistence, artifact read/list validation, traversal/external/scheme/oversize rejection, unsafe path rejection, corrupted manifest handling, and packaged runner discovery. |
...
| `pnpm package:desktop:internal` | pass | Built the release desktop binary with `tauri build --no-bundle --ci`, assembled `dist/private-beta/orynt-desktop-0.1.0-linux-x64.tar.gz`, wrote `SHA256SUMS`, and verified archive contents include `orynt-desktop`, `orynt-runner`, release notes, smoke checklist, and compiled package outputs. SHA256: `f11c4d482c47540096642971e6443246d7e3cde5b92b594e6c55068715a5c05a`. |
| `sha256sum -c SHA256SUMS` from `dist/private-beta` | pass | Final tarball checksum matched `SHA256SUMS`. |
| Clean extraction/package content smoke | pass | Extracted the final tarball to `/tmp/orynt-release-final.8UZ2S4/orynt-desktop-0.1.0-linux-x64`; verified executable bit, release manifest, runner scripts, release notes, smoke checklist, required compiled package files, and absence of the self-referential readiness audit. |
| Packaged sidecar repository smoke | pass | Ran extracted `orynt-runner/scripts/desktop-repository-run.mjs` with `node --import orynt-runner/scripts/register-extensionless-esm-loader.mjs` against a disposable git repository; output status was `pass`, event count was 30, and manifest-owned contract, event log, verifier input/result, redacted log, memory store, and replay plan files existed. |
| Packaged binary process smoke | not rerun after Orynt packaging rename | Visual packaged app launch and onboarding confirmation remain manual because this environment has no interactive display capture. |
...
Optional real Codex smoke was not used as a release gate. `ORYNT_RUN_REAL_CODEX=1 pnpm walkthrough:real-codex` can use local auth, network, and model budget, so it should remain an explicit local operator check rather than a default beta validation command.
