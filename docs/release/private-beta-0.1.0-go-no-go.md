# CodePawl Private Beta 0.1.0 Go/No-Go
...
- Path: `dist/private-beta/codepawl-desktop-0.1.0-linux-x64.tar.gz`
- Checksum file: `dist/private-beta/SHA256SUMS`
- SHA256: `9f14b5db3a834d343937c9014bf402f5bad07815aff35864cc20ea4f3e048599`
- Manifest: `dist/private-beta/codepawl-desktop-0.1.0-linux-x64/RELEASE_MANIFEST.json`
...
| --- | --- | --- |
| Tarball exists | verified | `dist/private-beta/codepawl-desktop-0.1.0-linux-x64.tar.gz` exists after `pnpm package:desktop:internal`. |
| Checksum matches | verified | `sha256sum -c SHA256SUMS` passed from `dist/private-beta`. |
| Release manifest fields | verified | Manifest has product `CodePawl Desktop`, version `0.1.0`, target `linux-x64`, binary `codepawl-desktop`, runner root `codepawl-runner`, unsigned distribution, disabled updater, repository-only scope, app data paths, and smoke checklist pointer. |
| Sidecar files included | verified | Archive contains `codepawl-runner/scripts/desktop-repository-run.mjs`, `register-extensionless-esm-loader.mjs`, and compiled `@codepawl/*/dist/index.js` package outputs required by the runner. |
| Clean extraction | verified | Extracted to `/tmp/codepawl-release-final.OII2qU/codepawl-desktop-0.1.0-linux-x64`; binary, manifest, runner scripts, and smoke checklist were present. The self-referential readiness audit is intentionally not packaged. |
...
| `pnpm --filter @codepawl/desktop test` | pass | 1 file, 46 tests. Covers onboarding, repository path gating, provider setup/preflight UI, persisted run listing, evidence viewer states, disabled surfaces, and no live billing copy. |
...
| --- | --- | --- |
| App launch | partially verified | `timeout 8 env XDG_CONFIG_HOME=/tmp/codepawl-release-gui-config WEBKIT_DISABLE_DMABUF_RENDERER=1 ./codepawl-desktop` stayed alive until timeout with no crash output. Visual window confirmation is manual because this environment has no interactive display capture. |
| First-run onboarding | verified by automated UI test; visual manual check required | `pnpm --filter @codepawl/desktop test` covers first-run private beta onboarding text, dismissal persistence, and run blocking before onboarding. |
...
| Repository run | verified through packaged sidecar | Final clean-extraction sidecar smoke ran `codepawl-runner/scripts/desktop-repository-run.mjs` against a disposable git repository and returned `status: pass`, `eventCount: 30`. |
...
- Real Codex smoke: skipped by design. It requires local Codex authentication, network/model budget, and explicit operator opt-in. Use `CODEPAWL_RUN_REAL_CODEX=1 pnpm walkthrough:real-codex` only as a manual operator check.
...
- Provider setup is Codex CLI readiness only; raw secrets are not stored by CodePawl app data, and OS keychain-backed BYOK storage is not implemented.
...
1. Receive `codepawl-desktop-0.1.0-linux-x64.tar.gz` and `SHA256SUMS` through the trusted internal channel.
...
3. Extract and run:
   ...
   ```bash
   tar -xzf codepawl-desktop-0.1.0-linux-x64.tar.gz
   cd codepawl-desktop-0.1.0-linux-x64
   ./codepawl-desktop
   ```

4. Complete `docs/private-beta-release-smoke.md` from the extracted archive.
5. Keep `codepawl-runner/` next to `codepawl-desktop`; repository runs depend on that sidecar bundle.
6. Use a disposable or intentionally selected local git repository for beta testing.
...
- To stop using this beta, quit CodePawl and delete the extracted `codepawl-desktop-0.1.0-linux-x64/` directory.
- Local app data on Linux is under `$XDG_CONFIG_HOME/com.codepawl.desktop` or `~/.config/com.codepawl.desktop`.
- To reset local beta state, quit CodePawl, optionally archive the app data directory for debugging, then delete that app data directory.
