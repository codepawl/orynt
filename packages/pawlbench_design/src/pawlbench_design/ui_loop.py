...
SUPPORTED_PATCH_MODES = {"instruction_only", "deterministic_patch", "oracle_patch", "manual_patch"}
...
def run_loop(config: LoopRunConfig) -> dict[str, Any]:
    ...
    for task in tasks:
        ...
        if config.include_noop_baseline:
...
def run_loop_task(task: dict[str, Any], config: LoopRunConfig, feature_group: str, *, mode: str) -> dict[str, Any]:
    ...
    write_instruction_artifacts(instruction, config.output_dir.expanduser().resolve())
    ...
    patch_result = apply_patch_mode(task, instruction, mode=mode, work_dir=work_dir)
    ...
    after_screen_id = str(task["before_screen_id"]) if mode in {"instruction_only", "manual_patch", "no_op"} else f"{task['base_screen_id']}__after"
    ...
    pass_threshold = 0.05 if mode == "deterministic_patch" else 0.0
    improvement_passes = (
...
def critique_to_instruction(task: dict[str, Any], *, feature_group: str = "metrics") -> dict[str, Any]:
    ...
    payload = {
        ...
        "work_contract": {
            ...
            "Context": "The artifact is a copied local HTML fixture with rendered before metrics and critique JSON.",
...
def instruction_markdown(instruction: dict[str, Any]) -> str:
    ...
    for item in instruction["expected_visual_improvement_criteria"]:
        lines.append(f"- {item}")
    lines.extend(["", "## Codex-Compatible Work Contract", "", "### Goal", str(contract["Goal"]), "", "### Context", str(contract["Context"]), "", "### Constraints"])
...
def write_instruction_artifacts(instruction: dict[str, Any], output_dir: Path) -> None:
    ...
    instruction["artifact_paths"] = {"json": str(json_path), "markdown": str(md_path)}
    write_json(json_path, instruction)
    md_path.write_text(instruction_markdown(instruction), encoding="utf-8")


def apply_patch_mode(task: dict[str, Any], instruction: dict[str, Any], *, mode: str, work_dir: Path) -> dict[str, Any]:
    before_html = Path(str(task["before_html_path"]))
...
def aggregate_loop_reports(
    ...
    *,
    ...
    non_oracle = [report for report in main if not report.get("oracle_excluded_from_non_oracle_claims")]
    deltas = [float(report.get("critic_delta") or 0.0) for report in main if report.get("patch_success")]
    noop_deltas = [float(report.get("critic_delta") or 0.0) for report in noops if report.get("patch_success")]
    success_count = sum(1 for report in main if report.get("improvement_passes_local_threshold"))
    easy = [report for report in main if report.get("difficulty") == "easy"]
    ...
    accessibility_rate = rate(sum(1 for report in main if report.get("accessibility_regression")), len(main))
    responsive_rate = rate(sum(1 for report in main if report.get("responsive_regression")), len(main))
    deterministic_or_manual_improves_easy = bool(easy) and rate(easy_success, len(easy)) >= 0.5
    passed_gate = (
        bool(main)
        and not false_noop
        and accessibility_rate <= 0.1
        and responsive_rate <= 0.1
        and patch_mode in {"deterministic_patch", "manual_patch"}
        and deterministic_or_manual_improves_easy
    )
    recommendation = recommendation_for_next_stage(patch_mode, deterministic_or_manual_improves_easy, false_noop, accessibility_rate, responsive_rate)
    return {
        "schema_version": LOOP_REPORT_SCHEMA_VERSION,
        "valid": bool(main),
        ...
        "success_rate": rate(success_count, len(main)),
        ...
        "difficulty_breakdown": breakdown(main, "difficulty"),
        "patch_mode": patch_mode,
        "examples": {
            "wins": [example_summary(report) for report in sorted(main, key=lambda row: float(row.get("critic_delta") or 0.0), reverse=True)[:5]],
            "failures": [example_summary(report) for report in sorted(main, key=lambda row: float(row.get("critic_delta") or 0.0))[:5] if not report.get("improvement_passes_local_threshold")],
        },
...
def recommendation_for_next_stage(patch_mode: str, easy_ok: bool, false_noop: bool, accessibility_rate: float, responsive_rate: float) -> str:
    if false_noop:
    ...
    if patch_mode == "deterministic_patch" and easy_ok:
    ...
    if patch_mode == "oracle_patch":
        return "treat_as_upper_bound_only_then_add_non_oracle_manual_or_deterministic_workflow"
...
def build_loop_report_from_task_dir(output_dir: Path, *, dataset_dir: Path | None = None, patch_mode: str = "deterministic_patch") -> dict[str, Any]:
