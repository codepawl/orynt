...
test('renders the Orynt landing page', async ({ page }) => {
  await page.goto('/');

  await expect(page).toHaveTitle(/Orynt/);
  await expect(page.getByRole('heading', { level: 1, name: 'Give AI agents a working brain.' })).toBeVisible();
  await expect(page.getByRole('navigation', { name: 'Primary navigation' })).toBeVisible();
  await expect(page.getByRole('link', { name: 'Download Orynt' })).toHaveAttribute('href', '/access');
});
