"""CLI for UI-JEPA M1 random-block screenshot JEPA training."""
...
from pawl_jepa.m1 import M1TrainConfig, train_m1


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="ui-jepa-m1-train",
        description="Train the M1 random-block screenshot JEPA baseline and write its report.",
    )
    ...
    parser.add_argument("--target-area-min", type=float, default=0.06)
    parser.add_argument("--target-area-max", type=float, default=0.18)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"ui-jepa-m1-train: {exc}", file=sys.stderr)
        return 2
    progress.done("M1 training complete")
